...
public class PackageTest {
    ...
    public void testPackageRaiseIfFileStreamNull() {
        ...
        Package p = new Package("test.gem", bos, "myrepo");
    }

    @Test(expected = IllegalArgumentException.class)
    public void testPackageRaiseIfRepositoryNull() {
        byte[] b = {((byte)9)};
        Package p = new Package("test.gem", new ByteArrayInputStream(b), null);
    }
    @Test
    public void testPackageFilestreamGetter() {
        byte[] b = {((byte)9)};
        Package p = new Package("test.gem", new ByteArrayInputStream(b), "myrepo");
